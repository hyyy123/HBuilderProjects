	...
	for(var i = 0; i < facilityJson.length; i++) {
		if(facilityJson[i].name == elename) {
			...
			for(var j = 0; j < facilityJson[i].son.length; j++) {
				...
				cardhtml += '<p class="faitemname" id=' + facilityJson[i].son[j].data + '>' + facilityJson[i].son[j].name + '<a class="pull-right" onclick="getChartData(\'' + nowcode + '\',this)">查看图表</a> </p>';

				cardhtml += '<ul class="clearfix" >';
				if(imgid == 1) {

					cardhtml += '<li class="clearfix"><p class="pull-left">运行状态</p><p class="pull-left value41"></p></li>';
				} else if(imgid == 5) {

					cardhtml += '<li class="clearfix pull-left"><p class="pull-left">运行状态</p><p class="pull-left value109"></p></li>';
					cardhtml += '<li class="clearfix pull-left"><p class="pull-left">运行频率(Hz)</p><p class="pull-left value110"></p></li>';
				} else {
...
//图表数据接口
function getChartData(nowcode, param) {
	jQuery(".faitem #main1").remove();
	jQuery(".faitem").has(param).append('<div id="main1"> </div>');
	//	alert(jQuery(".faitem").has(param).length)
