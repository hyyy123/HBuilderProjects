...
function getChartNew(option, func) {
    ...
    if (sort == 1) {
        ...
        postData = "sort=" + sort + "&type=" + type + "&area_id=" + area_id + "&codes=" + device_id + "&start_date_time=" + start_date_time;
    }
    else if (sort == 2) {
...
//获取预警列表
function getWarnsafeList(p, ps, type, code, alarm_type, func) {
    var url = http + "/tools/EcmAlarm.ashx?action=get_list&p=" + p + "&ps=" + ps + "&code=" + code + "&type=" + type + "&alarm_type=" + alarm_type;
    //console.log(url)
    getAjax(url, true, false, function (data) {
        if (func != undefined) {
            func(data);
