...
function initEasyDropDown(selector) {
    $(selector).easyDropDown({ cutOff: 10 });
...
function initSidebar() {
    ...
    $(".sidebar-close").click(function () {
        $(".sidebar-right").animate({ "right": -220 })
    });
...
function jsprint(msgtitle, url, callback, option) {
    if (msgtitle != "") {
        layer.msg(msgtitle, { icon: option }, function () {
            if (url == "back") {
                ; history.back(-1);
            } else if (url != "") {
                location.href = url;
...
function jsTip(msgcontent, callback, option) {
    if (option != undefined) {
        if (option.icon == undefined) {
            ...
            layer.alert(msgcontent, { icon: option.icon });
        }
    }
    else {
        layer.alert(msgcontent);
...
function getAjax(url, isCache, isLoading, func, errorFuc) {
    if (typeof isLoading == "boolean") {
    ...
    else if (typeof isLoading == "object") {
        var doc = isLoading.doc;
        var width = isLoading.width;
        var imgurl = isLoading.imgurl;
        if (imgurl == "" || imgurl == undefined) {

            var type = isLoading.type;
            if (type != undefined && type != "") {
                switch (type) {
                    case 1:
                        imgurl = '/templates/ecm/assets/img/loading.gif';
                        break;
                    case 2:
                        imgurl = '/templates/ecm/assets/img/loading.gif';
                        break;
        ...
        var $loadingDoc = $('<img src=\"' + imgurl + '\" width=\"' + width + 'px\" />');
        $.ajax({
            type: "get",
            url: url,
            dataType: 'json',
        ...
        var $loadingDoc = $('<img src=\"/templates/ecm/assets/img/loading.gif\" width=\"30px\" />');
        $.ajax({
            type: "get",
            url: url,
            dataType: 'json',
...
function status(sts, name) {
    if (parseInt(sts) == 1) {
        $(name).removeClass("lightgrey");
        $(name).addClass("lightgreen");
        //$(".value101")
    } else {
        $(name).removeClass("lightgreen");
        $(name).addClass("lightgrey");
    }
}
//写cookies  
