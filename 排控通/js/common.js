...
function infoMsg(param) {
	...
	html += '</div>';
	html += '</div>';
	html += '<div class="mui-popup-buttons"><span class="mui-popup-button mui-popup-button-bold">确定</span></div>';
	html += '</div>';

	jQuery("body").append(html);

	$(".mui-popup-button").click(function() {
		$(".mui-popup").remove();
	})
}
//下一节课时间
function nextSection(timeH, timeM) {
	...
	for(var i = 0; i < courseArray.length; i++) {
		var stime = courseArray[i].start_hour + ":" + courseArray[i].start_minute;
		var etime = courseArray[i].end_hour + ":" + courseArray[i].end_minute;
